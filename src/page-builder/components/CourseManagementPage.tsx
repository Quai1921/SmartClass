...
import { BookOpen, Plus, ArrowLeft, RotateCcw } from 'lucide-react';
...
export const CourseManagementPage: React.FC = () => {
  ...
  const handleRefreshData = async () => {
    try {
      ...
      setTimeout(() => {
        // Only show success if no error alert is currently showing
        if (!showAlert) {
      ...
      showAlertWithMessage('Error durante la actualización', 'error');
    }
  };
  ...
  const filteredCourses = courses.filter(course => {
    ...
    const matchesStatus = statusFilter === 'Todos los estados' ||
                         ...
                         (statusFilter === 'En Revisión' && course.status === 'IN_REVIEW');
    ...
    <ToastProvider>
      <div className="min-h-screen bg-gray-900 text-white -m-6">
            ...
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  disabled={loading}
                  className="inline-flex items-center px-3 py-2 bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 focus:ring-offset-gray-800"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          {/* Course Grid with conditional loading */}
          {loading ? (
