...
import type { TeacherForSelection } from "./get-teachers-for-selection";

interface ExportTeachersParams {
    // Basic filters
    search?: string;
    status?: string;
    performance?: number;
...
export const exportTeachers = async (params: ExportTeachersParams) => {
    try {
        ...
        // Add advanced filters - use consistent parameter names
        ...
        // Add selected teachers from multi-select
        if (params.selectedTeachers && params.selectedTeachers.length > 0) {
            params.selectedTeachers.forEach(teacher => queryParams.append('teacherIds', teacher.id));
        }
        ...
        const response = await smartClassAPI.get(`/institutions/teachers/export-csv?${queryParams.toString()}`, {
            headers: {
        ...
        const filename = params.template
            ? `docentes_plantilla_${timestamp}.csv`
            : `docentes_${timestamp}.csv`;
